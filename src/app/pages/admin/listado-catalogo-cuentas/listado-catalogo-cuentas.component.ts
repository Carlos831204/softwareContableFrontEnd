...
export class ListadoCatalogoCuentasComponent implements OnInit {
  ...
  private obtenerCuentasCatalogo(){
    ...
    (error)=>{
      swal('Error !!', 'Error al cargar el catálogo de cuentas', 'error');
    })
  }

  obtenerCuentasPorCodigo(codigo:string){
    this.catalogoCuentaServicio.obtenerCuentaPorCodigo(codigo).subscribe(dato =>{
      this.obtenerCuentasCatalogo();
    })
  }
